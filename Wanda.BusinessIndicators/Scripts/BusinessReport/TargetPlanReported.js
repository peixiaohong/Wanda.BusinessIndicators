...
function GetTreeData() {
    ...
    WebUtil.ajax({
        async: true,
        url: "/TargetController/GetTargetPlanTreeDataJosn",
        args: { SysID: SystemID, FinYear: SYear },
...
function Getlist() {
    ...
    WebUtil.ajax({
        async: true,
        url: "/TargetController/GetAllTargetApprove",
        args: { SysID: SystemID, FinYear: SYear },
        successReturn: function (List) {
            BangList(List);
            Fake();
...
function BangList(List) {
    ...
    if (List.length > 0) {
        for (var i = 0; i < List.length; i++) {
            ...
            var finYear = $.base64.btoa($("#ddlSystem").val());//base64加密
            //首先判断数据库字段是否有值。无值则不显示。
            if (List[i].ReportApprove) {
                if (List[i].Group > 0) // 根据Group判断需要向下合并几个TD。
                {
                    index++;
                    th += "<tr><td class=\"Td_Center\" rowspan=" + List[i].Group + " ><input name=\"ChecBoxs\" type=\"checkbox\" value=" + List[i].BusinessID + " /><td class=\"Td_Center\" rowspan=" + List[i].Group + " >" + index + "</td>";
                    th += "<td class=\"Td_Center\" rowspan=" + List[i].Group + "><a target=\"_blank\" href=\"../SystemConfiguration/TargetCollectDisplay.aspx?sysID=" + sysid + "&FinYear=" + finYear + "\">" + List[i].SystemName + "</a></td><td class=\"Td_Center\">" + List[i].CreatTime + "</td><td class=\"Td_TrueLeft\">";
                ...
                {
                    th += "<tr> <td class=\"Td_Center\" rowspan=" + List[i].Group + "><a target=\"_blank\" href=\"../SystemConfiguration/TargetCollectDisplay.aspx?sysID=" + sysid + "&FinYear=" + finYear + "\">" + List[i].SystemName + "</a></td><td class=\"Td_Center\">" + List[i].CreatTime + "</td><td class=\"Td_TrueLeft\">";
                    th += "<td class=\"Td_Center\">" + List[i].CreatTime + "</td><td class=\"Td_TrueLeft\">";
                    for (var n = 0; n < List[i].list.length; n++) {
                        td += CreatList(List[i].list[n]);
                    }
...
function DownExcel() {
    ...
    if (BusID != "") {
        window.open("/AjaxHander/DownFileList.ashx?BusinessID=" + BusID + "&BusinessType=" + "系统生成Excel" + "&IsMonth=" + 2);
    } else {
        alert("请选择系统！");
        //$('input[name="ChecBoxs"]').each(function () {
        //    BusID += $(this).val() + ",";
        //});
        //if (BusID.length > 0) {
        //    BusID = BusID.substring(0, BusID.length - 1);
