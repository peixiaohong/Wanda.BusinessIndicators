If Object_Id('dbo.GetMonthlyReportDetailList_Result') Is Not Null
Drop Procedure dbo.GetMonthlyReportDetailList_Result
Go

Create PROCEDURE [dbo].[GetMonthlyReportDetailList_Result] 
...
DECLARE @IsHaveArea INT--大于0有区域（表示月报可分区域分批上传）
...
--获取最新批次ID,根据最新批次ID查找MonthlyReportID,然后关联出所有的月报明细
	BEGIN 
		...
		SET  @SystemBatchID=(SELECT TOP 1 SystemBatchID FROM A_MonthlyReport WHERE SystemID=@SystemID AND FinYear=@FinYear AND FinMonth=@FinMonth AND TargetPlanID=@TargetPlanID ORDER BY CreateTime DESC);
	   SELECT a.[ID]
      , a.[SystemID]
      ,a.[FinYear]
      ,a.[FinMonth]
...
ELSE 
	...
	SET @lastMonthlyReportID=(SELECT TOP 1 ID FROM A_MonthlyReport WHERE  SystemID=@SystemID AND FinYear=@FinYear AND FinMonth=@FinMonth AND TargetPlanID=@TargetPlanID ORDER BY CreateTime DESC)
		SELECT [ID]
      ,[SystemID]
      ,[FinYear]
      ,[FinMonth]
