    ...
    drowRowData = function (_rows, _cols, _level, _pid) {
        ...
        for (var i = 0; i < _rows.length; i++) {
            ...
            for (var j = 0; j < _cols.length; j++) {
                ...
                //节点图标
                if (j == folderColumnIndex) {
                    if (row.children) { //有下级数据 row.children.length> 0
                ...
                if (col.handler) {
                    ...
                    //添加判断，裂名 ：
                    if (col.dataField == "ReportApprove") {

                        if (row[col.dataField] != null && row[col.dataField] != undefined && row[col.dataField] != "") {
                            var td = "";//列数据(审批列)

                            //这里做下处理
                            var ApproveList = TreeGrid.str2json(row[col.dataField]);

                            td += CreatList(ApproveList);
                    ...
                    else if (col.dataField == "WFStause") {

                        if (row[col.dataField] == "Approved") {
                            s += "  审批通过   </td>";
                        } else if (row[col.dataField] == "Progress") {
                            s += " <span style=\"color:red\"> 审批中 </span></td>";
                        } else if (row[col.dataField] == "Draft") {
                            s += "<span style=\"color:red\">退回</span></td>";
                        } else if (row[col.dataField] == "Cancel") {
                            s += "<span style=\"color:red\">作废 </span></td>";
                        }
...
//月报公司的处理，跳转到其它页面
function customMonthSysName(row, col) {
    ...
    var IsLatestVersion = false; // 是否包含审批中 


    if (row["Category"] > 0) {

        if (row["WFStause"] == "Approved")
            IsLatestVersion = false;
        else
            IsLatestVersion = true;

        if (row["ReportApprove"] != null && row["ReportApprove"] != "" && row["WFStause"] !="Cancel") {
            if (row["Category"] == 1) {

                name = "<a target=\"_blank\" href=\"../BusinessReport/TargetRpt.aspx?_sysid=" + row["SysID"] + "&_finYear=" + SYear + "&_finMonth=" + $("#ddlMonth").val() + "&IsLastestVersion=" + IsLatestVersion + "\">" + name + "</a>";

            } else if (row["Category"] == 2) {
                if (row["Name"] != "合并后项目") {
                    name = "<a target=\"_blank\" href=\"../BusinessReport/ProMonthReport.aspx?_sysid=" + row["SysID"] + "&_finYear=" + SYear + "&_finMonth=" + $("#ddlMonth").val() + "&IsLastestVersion=" + IsLatestVersion + "\">" + name + "</a>";
                }
            } else if (row["Category"] == 3) {

                name = "<a target=\"_blank\" href=\"../BusinessReport/TargetGroupRpt.aspx?_sysid=" + row["SysID"] + "&_finYear=" + SYear + "&_finMonth=" + $("#ddlMonth").val() + "&IsLastestVersion=" + IsLatestVersion + "\">" + name + "</a>";
            } else if (row["Category"] == 4) {

                name = "<a target=\"_blank\" href=\"../BusinessReport/TargetDirectlyRpt.aspx?_sysid=" + row["SysID"] + "&_finYear=" + SYear + "&_finMonth=" + $("#ddlMonth").val() + "&IsLastestVersion=" + IsLatestVersion + "\">" + name + "</a>";
            }
        }
        

        //name = "<a target=\"_blank\" href=\"../SystemConfiguration/TargetCollectDisplay.aspx?SystemID=" + row["SysID"] + "&FinYear=" + SYear + "\">" + name + "</a>";
...
//分解指标的处理，跳转到其它页面
function customTargetSysName(row, col) {
    ...
    if (row["WFStause"] == "Approved") {
        ...
        //if (row["Category"] > 0) {
            ...
            if (row["Category"] == 3) {
                ...
                name = "<a target=\"_blank\" href=\"../BusinessReport/TargetPlanDetailRpt.aspx?SystemID=" + row["SysID"] + "&FinYear=" + SYear + "\">" + name + "</a>";
            } else {
                name = "<a target=\"_blank\" href=\"../SystemConfiguration/TargetCollectDisplay.aspx?SystemID=" + row["SysID"] + "&FinYear=" + SYear + "\">" + name + "</a>";
            }
        //}
