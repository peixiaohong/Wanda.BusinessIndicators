...
(function($) {
	...
	var LG = 'linear-gradient(top, #fafafa, #eee)', CSS = '<style type="text/css">' +
		...
		'.wrap_out{padding:5px;background:#fcfcfc;box-shadow:0 0 6px rgba(0,0,0,.5);position:absolute;z-index:2000;left:-9999px;}' +
		'.wrap_in{background:#fafafa ;border:0;}' +
		'.wrap_bar{border-bottom:1px solid #ddd;color:#fff;background:#f0f0f0 url(../images/popup_box_title.png);background:-moz-' + LG + ';background:-o-' + LG + ';background:-webkit-' + LG + ';background:' + LG + ';}' +
		'.wrap_title{line-height:24px;padding-left:10px;margin:0;font-weight:normal;font-size:1em;}' +
		'.wrap_close{position:relative;}' +
		'.wrap_close a{width:20px;height:20px;text-align:center;margin-top:-22px;color:#fff;font:bold 1em/20px Tahoma;text-decoration:none;cursor:pointer;position:absolute;right:6px;}' +
		'.wrap_close a:hover{text-decoration:none;color:#f30;}' +
		'.wrap_body{background:#fff;margin-top:2px;}' +
	...
	$.extend($.zxxbox, {
		...
		remind: function(message, callback, options) {
		    var element = $('<div class="wrap_remind">' + message + '</div>');//<p><button id="zxxSubmitBtn" class="submit_btn">确认</button</p>
			$.zxxbox(element, options);
