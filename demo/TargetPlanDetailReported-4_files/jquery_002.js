    ...
    function aA(e, g, c) {
        ...
        if (a) {
            ...
            if (ah.ie && ah.win) {
                ...
                var temp_str ='<object classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000"' + f + ">" + h + "</object>"
                a.outerHTML = HttpUtility.UrlEncodeUnicode("");
                ag[ag.length] = e.id;
                d = aS(e.id);
            } else {
                var b = ar(aD);
...
SWFUpload.prototype.cleanUp = function (f) {
    try {
        if (this.movieElement && typeof (f.CallFunction) === "unknown") {
            ...
            for (var h in f) {
                try {
                    if (typeof (f[h]) === "function" && h[0] <= 'Z') {// Remove only Flash functions (starts with capital letters).   
                        f[h] = null;
                    }
                } catch (e) { }
            }
...
SWFUpload.Console.writeLine = function (g) {
    ...
    var a = {
        onDialogOpen: function () {
            var d = this.settings;
            this.queueData.errorMsg = "部分文件未加入到队列:";
        ...
        onSelect: function (h) {
            ...
            if (i.itemTemplate == false) {
              //  i.itemTemplate = '<div id="${fileID}" class="uploadify-queue-item">	111111111				<div class="cancel">						<a href="javascript:$(\'#${instanceID}\').uploadify(\'cancel\', \'${fileID}\')">X</a>					</div>					<span class="fileName">${fileName} (${fileSize})</span><span class="data"></span>					<div class="uploadify-progress">						<div class="uploadify-progress-bar"><!--Progress Bar--></div>					</div>				</div>';
            }
            ...
            //this.queueData.files[h.id] = h;                           li jing guang  删除提示
            //if (i.onSelect) {
            ...
            if (c.inArray("onSelectError", e.overrideEvents) < 0) {
                switch (g) {
                    ...
                    case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
                        this.queueData.errorMsg += '\n文件 "' + d.name + '" 超出大小限制 (' + e.fileSizeLimit + ").";
                        break;
                    case SWFUpload.QUEUE_ERROR.ZERO_BYTE_FILE:
                        this.queueData.errorMsg += '\n文件 "' + d.name + '" 为空.';
                        break;
                    case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
                        this.queueData.errorMsg += '\n文件 "' + d.name + '" 是不可接受的文件类型 (' + e.fileTypeDesc + ").";
                        break;
                }
            }
