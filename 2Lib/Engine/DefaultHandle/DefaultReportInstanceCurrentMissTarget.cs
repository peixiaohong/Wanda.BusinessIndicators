...
namespace LJTH.BusinessIndicators.Engine
{
    ...
    public class DefaultReportInstanceCurrentMissTarget : IReportInstanceCurrentMissTarget
    {
        ...
        public List<DictionaryVmodel> GetCurrentMissTargetRptDataSource(ReportInstance RptModel, C_System sys)
        {
            ...
            if (element.Elements("CurrentMisstarget").Elements("Group") != null)
            {
                List<XElement> Groups = element.Elements("CurrentMisstarget").Elements("Group").ToList();
            ...
            if (element.Elements("CurrentMisstarget").ToList().Count > 0)
            {
                XElement subElement = element.Elements("CurrentMisstarget").ToList()[0];
            ...
            if (ReportDetails != null && ReportDetails.Count > 0)
            {
                ...
                MissTargetTextList = ReportDetails.Select(x => x).ToList();
            ...
            foreach (var Group in GroupList.OrderBy(G => G.Senquence))
            {
                ...
                Vmodel.ObjValue = FormartVData(Group, "CurrentMisstarget", Group.TargetName);
                Vmodel.Mark = "Group";
    ...
    /// 直属公司的未完成原因
    /// </summary>
    public class Directly_ReportInstanceCurrentMissTarget : IReportInstanceCurrentMissTarget
    {
