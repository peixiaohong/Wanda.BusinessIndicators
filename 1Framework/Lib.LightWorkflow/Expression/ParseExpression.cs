...
namespace Wanda.Lib.LightWorkflow.Expression
{
    ...
    public class ParseExpression
    {
        ...
        public List<string> GetVariables() {
            return this.Variables.ToList();
        }

        private EXP_TreeNode DoVariable()
        {
            int nPos = _Position;
            char ch = _ExpressionChars[++_Position];

            //OutputID(Operation_IDs.OI_Variable, "$", nPos);
            //EXP_TreeNode node = NewTreeNode();

            //node._Position = nPos;
            //node._OperationID = Operation_IDs.OI_Variable;
            //node._Params = new ArrayList();
            //node._Params.Add(DoString());
            ...
            if (Char.IsLetter(ch))
            {
                ...
                EXP_TreeNode param = NewTreeNode();
                param._Value = strID;
                param._OperationID = Operation_IDs.OI_STRING;
                node._Params.Add(param);
                OutputID(Operation_IDs.OI_STRING, strID, _Position);


                this.Variables.Add("$" + strID);

                return node;
            }
            else
            {
                throw ParsingException.NewParsingException(Parse_Error.peInvalidChar, _Position, "");
            }
        }
            ...
            if (node != null)
            {
                try
                {
                    switch (node._OperationID)
                    {
                        ...
                        case Operation_IDs.OI_Variable:
                            {
                                ParamObject[] po = GetParamArray(node._Params);
                                oValue = CalculateFunction("getValue", po, this);
                            }
                            break;
